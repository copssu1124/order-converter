...
def fonts_dir(style):
    """자막 폰트가 있는 폴더. 맥·윈도우·리눅스 어디서 실행해도 찾아낸다."""
    if style.get("fontsdir"):
        return os.path.abspath(style["fontsdir"])
    here = os.path.dirname(os.path.abspath(__file__))
    for cand in (os.path.join(here, "..", "fonts"),      # 스킬로 설치했을 때
                 os.path.join(here, "fonts"),
                 os.path.join(here, "..", "..", "ui")):  # 저장소에서 바로 쓸 때
...
def build(spec, workdir):
    ...
    vf = f"subtitles={ass}:fontsdir={fonts_dir(style)}"
