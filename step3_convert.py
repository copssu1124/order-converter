...
기타등급표 = {}        # 신규 등급 택배사: {'위플': {'P': '3182'}, '원준': ..., '카몬드'/'올담'/'용차'}
                       #   매칭표에 해당 시트가 있을 때만 채워짐 → 없으면 기존(착불) 동작 유지

# 신규 등급 택배사 시트명 후보 (매칭표 시트명 앞뒤 공백 무시하고 탐색)
기타등급시트 = (('위플', ('위플택배비', '위플')), ('원준', ('원준택배비', '원준')),
              ('카몬드', ('카몬드택배비', '카몬드')), ('올담', ('올담택배비', '올담')),
              ('용차', ('용차택배비', '용차')))
...
# 요금 택배사(배송비합계가 있어야 정상). 다모아는 착불이라 배송비 빈칸이 정상.
#   원준/위플은 택배비 시트가 있으면 요금 택배사로 승격(_load_mapping에서 갱신).
기본요금택배사 = frozenset({'씨제이', '대신', '대신낱개', '대신택배', '로젠', '천일'})
요금택배사 = set(기본요금택배사)
...
def 변환시도(키, 수량):
    ...
    # 마켓 꼬리표 제거 폴백 — 벗긴 키가 매핑표에 '있을 때만' 채택(오탐 없음)
    #   /1,800원/1개(ESM) · (+4900원)(추가금) · -N개(11번가 등) · /N개  — 조합도 순차 제거
    후보들 = []
...
def 택배비조회(상품키):
    ...
    if 일반키 in 판매비표:                       # 1순위: 일반 키 (형식 정상일 때만)
        ...
        if carrier != '?':                       # '*' 없는 깨진 값은 일반으로 안 씀 → 천일 폴백
            ...
            # 신규 등급 택배사(위플/원준/카몬드/올담/용차) — 시트가 로드된 경우에만 등급 추출
            기타등급 = fee.strip().upper() if 기타등급표.get(carrier) else None
...
def _load_mapping(mapping_file, log):
    ...
    # 신규 등급 택배사(위플/원준/카몬드/올담/용차) — 시트명 앞뒤 공백 무시(' 카몬드택배비' 대응)
    기타등급표 = {}
    try:
        with pd.ExcelFile(mapping_file) as _xf:      # with: 파일 핸들 즉시 해제(잠김 방지)
            시트명맵 = {str(s).strip(): s for s in _xf.sheet_names}
    except Exception:
        시트명맵 = {}
    for 택배사명, 후보들 in 기타등급시트:
        표 = {}
        for 후보 in 후보들:
            if 후보 not in 시트명맵:
                continue
            try:
                원본 = pd.read_excel(mapping_file, sheet_name=시트명맵[후보], header=None)
            except Exception:
                continue
            for i in range(len(원본)):
                ...
                if pd.notna(등급) and pd.notna(금액):
                    try:
                        표[str(등급).strip().upper()] = str(int(금액))
                    except (ValueError, TypeError):
                        pass
            break
        if 표:                               # 시트가 있고 값도 있을 때만 등급 택배사로 승격
            기타등급표[택배사명] = 표
    # 요금 택배사 갱신: 기본 + 등급표가 실제로 채워진 신규 택배사(빈칸=빨강 검출 대상)
    요금택배사 = set(기본요금택배사) | set(기타등급표)
...
def convert(input_file, mapping_file, output_path=None, log=print, verbose=False):
        ...
        if 방식 == "매핑실패":
            ...
            if 경로 == '일반':
                ...
                if carrier == '다모아':
                    배송비합계 = None
                    fee = None
                elif carrier in ('원준', '위플') and not 기타등급표.get(carrier):
                    # 구 매칭표(택배비 시트 없음) → 기존처럼 착불(빈칸 정상, 하위호환)
                    배송비합계 = None
                    fee = None
                    ...
                    if m:                              # 숫자로 시작 → 이미 총액
                        ...
                        배송비합계 = None
                        fee = None
                # 택배등급: 천일박스 > 씨제이 > 로젠 > 대신택배 > 신규(위플/원준/카몬드/올담/용차)
                택배등급 = 박스등급 if 박스등급 else (씨제이등급 or 로젠등급 or 대신택배등급 or 기타등급 or '')
    ...
    for i, r in enumerate(결과목록):
        ...
        if r.get('is_수정택배비'):                # 수정택배비 행 → 택배사 칸도 라벤더(놓치지 않게)
            ...
            L_fill = MANUAL_FILL
        elif r['택배사'] in 요금택배사 and 빔:    # 요금 택배사인데 배송비합계 빈칸 = 가격누락 의심 → 수동확인(빨강)
            L_fill = MANUAL_FILL
        else:
            L_fill = COL_택배사_FILL
...
def 상세리포트(output_file, 사유목록):
    """변환 결과를 사람이 읽기 쉬운 이슈 목록으로 정리(읽기 전용, 변환 로직 미수정).
       - 사유목록: 분석_수동확인사유() 결과 [(엑셀행, 표시상품명, 사유)]
       - 추가로 '요금 택배사인데 배송비합계가 빈칸'(매핑 값 누락)인 행도 ②로 포함.
       반환: (총건수, 정상건수, 이슈목록)
         이슈 = {'행','상품명','수량','종류','문제','해결'} ; 종류='상품명없음'/'배송비없음'
    """
    문제_상품명없음 = '이 상품이 매핑표에 아예 없어요.'
    ...
    def 수량of(r):
        return ws.cell(row=r, column=11).value
    ...
    for 엑셀행, 상품, 사유 in 사유목록:                  # 수동확인(빨강) 행
        ...
        이슈_by_row[엑셀행] = {'행': 엑셀행, '상품명': str(상품), '수량': 수량of(엑셀행),
                              '종류': 종류, '문제': 문제, '해결': 해결}

    총 = 0
    for r in range(2, ws.max_row + 1):
        택배사 = ws.cell(row=r, column=12).value
        if 택배사 is None or str(택배사).strip() == '':
            continue
        총 += 1
        if r in 이슈_by_row:
            continue
        합계 = ws.cell(row=r, column=18).value
        if str(택배사).strip() in 요금택배사 and (합계 is None or str(합계).strip() == ''):
            상품 = ws.cell(row=r, column=9).value     # 회사상품명*수량 (값 누락 케이스)
            이슈_by_row[r] = {'행': r, '상품명': str(상품), '수량': 수량of(r),
                              '종류': '배송비없음', '문제': 문제_배송비없음,
                              '해결': 해결_배송비없음}
    wb.close()

    이슈 = [이슈_by_row[k] for k in sorted(이슈_by_row)]
    정상 = max(0, 총 - len(이슈))
    return 총, 정상, 이슈
...
def _fill_inplace(tpl, cfg, 행들, 발화주명, outp):
    ...
    for i, d in enumerate(행들):
        ...
        if cfg.get('hl_col') and d.get('_hl'):   # 대신낱개 등 강조 행 → 지정 열 연두
            ws.cell(row=rr, column=cfg['hl_col']).fill = _fill('92D050')
...
def _fill_rebuild(tpl, cfg, 행들, 발화주명, outp):
    ...
    for col, txt in (cfg.get('extra_headers') or {}).items():
        ows.cell(row=1, column=col, value=txt)   # 양식에 없는 열의 헤더(원준 L:총운임 등)
    for i, d in enumerate(행들):
        rr = i + 2
